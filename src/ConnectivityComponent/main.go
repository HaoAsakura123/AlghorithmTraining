...
func main() {
	...
	kolCom:=0
	for iterator:=vertex;iterator>0;iterator--{
		if _, ok:= mapGraph[iterator]; !ok{
			kolCom++
			s := Stack{}
			s.Push(iterator)
			mapGraph[iterator] = true
			resultat := DFS(&s, mapGraph, graph)
			defer fmt.Println(strings.Trim(fmt.Sprint(resultat), "[]"))
			defer fmt.Println(len(resultat))
		}
	}
	fmt.Println(kolCom)
}
